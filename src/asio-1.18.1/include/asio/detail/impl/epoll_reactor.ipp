...
void epoll_reactor::run(long usec, op_queue<operation>& ops)
{
  ...
  // 计算超时，只有timerfd不能被使用的时候菜检查timer的队列
  int timeout;

  //timeout =0 表示epoll_wait不会有任何阻塞。没有可以处理的任务就直接返回；通常如果全局队列中有多个任务还在等待的话，就会使用这种方式，不会被阻塞；
  //但是如果全局队列中没有任务的话，就会超时阻塞或者最多等待5分钟;
  if (usec == 0)
    timeout = 0;
  else
  {
...
# endif // defined(ASIO_HAS_TIMERFD)
  ...
  for (int i = 0; i < num_events; ++i)
  {
    ...
    else
    {
      ...
      // descriptor_state对象，保存了回调函数 + io本身的操作的对象，其实就是异步操作的上下文;
      descriptor_state* descriptor_data = static_cast<descriptor_state*>(ptr);
      // 判断当前这个任务是否已经在队列中了,如果没有就进行添加
...
{
    //默认超时时间最多不会超过5分钟,这将保证任对于系统任何的变化都能被检测到，在这个时间范围之内
