...
struct scheduler::task_cleanup
{
  ~task_cleanup()
  {
    ...
    {
        //将这部分的任务数 加到 全局的outstanding_work_上去
    ...
    // 因为要操作全局的队列了，就需要加锁
    lock_->lock();
    //这个说明当前线程要准备被阻塞了，所以会将自己的私有队列中的任务转交给全局队列，并且放一个task_operation_的任务到全局队列中，如果后面其他的线程运行到这个，就又可以执行epoll_wait的操作的
...
std::size_t scheduler::run(asio::error_code& ec)
{
  ...
  std::size_t n = 0;
  // 死循环，每次操作之前都会尝试去获得lock,内部等待的时候就会进行释放，而且scoped_lock是unique_lock，而非lock_guard
    ...
    const asio::error_code& ec)
{
  while (!stopped_)
  {
    if (!op_queue_.empty())
    {
      ...
      // task_operation: 表示epoll_wait的op
      if (o == &task_operation_)
      {
        task_interrupted_ = more_handlers;
        //如果有多个任务还在队列中并且非单线程，这个时候尝试唤醒其他的线程起来运行这些任务
        if (more_handlers && !one_thread_)
          wakeup_event_.unlock_and_signal_one(lock);
        else
          lock.unlock();

        //lock被解开,因为后面的所有的操作都是本线程的操作，不需要lock;
        ...
        // as soon as possible.
        // 运行epoll_wait操作，这个操作主要的目的是看有哪些事件已经可以被回调了;
        // 第一个参数: 如果有很多任务要被处理，就为0，负责是-1
        task_->run(more_handlers ? 0 : -1, this_thread.private_op_queue);
      }
      else
      {
          //非epoll_wait的任务，就是将asio当作线程池来使用;
          // task_result感觉作用真心不大啊，看不出来有什么用; 这个参数在windows下面有用;
        ...
        // 当前线程执行完当前的这个任务的时候，把当前线程的任务放到全局队列中去；这里面的思路应该是在运行当前op的时候会产生其他的需要做的操作，默认会放到当前的thread的私有队列中，当当前任务完成之后会存放到全局队列中来;这样可以方便其他的线程来消费
        ...
        //操作这种任务，每次都会返回到调度开始之处，用来统计成功完成的操作个数；
        return 1;
      }
    }
    else
