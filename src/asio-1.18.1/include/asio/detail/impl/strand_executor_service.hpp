...
public:
  ...
  struct on_invoker_exit
  {
    ...
    ~on_invoker_exit()
    {
      ...
      //把等待队列中的任务放到ready-queue中来
      this_->impl_->ready_queue_.push(this_->impl_->waiting_queue_);
      //如果当前的ready队列非空就表示当前locked为true, 如果已经为空的话，那么说明当前thread已经被工作全部完成;
  ...
  void operator()()
  {
    ...
    // 把当前的strand的impl加入到当前线程的调用stack上;
    ...
    Executor& ex, ASIO_MOVE_ARG(Function) function, const Allocator& a)
{
  ...
  // 假如我们已经在strand的调用stack上，那么函数就会被直接调用
